<template><div><h1 id="kafa集群原理设计-四-之-controller-选举和-partition-分配" tabindex="-1"><a class="header-anchor" href="#kafa集群原理设计-四-之-controller-选举和-partition-分配"><span>kafa集群原理设计（四）之 Controller 选举和 Partition 分配</span></a></h1>
<p>Kafka 集群中的 Controller 选举和 Partition 分配涉及 ZooKeeper 和 Kafka 两部分的协作。
ZooKeeper 负责协调和维护元数据，而 Kafka 负责具体的实现和业务逻辑。下面详细介绍启动过程
中的关键步骤，包括 ZooKeeper 如何触发 Controller 节点的创建，以及 Controller 如何实现
Partition 分配。</p>
<h3 id="_1-启动过程和-controller-选举" tabindex="-1"><a class="header-anchor" href="#_1-启动过程和-controller-选举"><span>1. 启动过程和 Controller 选举</span></a></h3>
<h4 id="zookeeper-启动" tabindex="-1"><a class="header-anchor" href="#zookeeper-启动"><span>ZooKeeper 启动</span></a></h4>
<p>当 ZooKeeper 启动时，每个 Kafka Broker 会连接到 ZooKeeper 并注册自身的信息。ZooKeeper
负责监控这些 Broker 的状态，并在 Controller 选举过程中起关键作用。</p>
<h4 id="kafka-broker-启动并注册" tabindex="-1"><a class="header-anchor" href="#kafka-broker-启动并注册"><span>Kafka Broker 启动并注册</span></a></h4>
<p>每个 Kafka Broker 启动时，会向 ZooKeeper 注册自身信息，并尝试创建 <code v-pre>/controller</code> 节点。</p>
<h3 id="_2-controller-选举" tabindex="-1"><a class="header-anchor" href="#_2-controller-选举"><span>2. Controller 选举</span></a></h3>
<h4 id="创建-controller-节点" tabindex="-1"><a class="header-anchor" href="#创建-controller-节点"><span>创建 <code v-pre>/controller</code> 节点</span></a></h4>
<p>在 Kafka 中，Controller 选举通过创建 ZooKeeper 的 <code v-pre>/controller</code> 节点实现。以下是 Kafka
源码中的关键部分：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="触发创建-controller-节点" tabindex="-1"><a class="header-anchor" href="#触发创建-controller-节点"><span>触发创建 <code v-pre>/controller</code> 节点</span></a></h4>
<p>每个 Kafka Broker 在启动时都会尝试创建 <code v-pre>/controller</code> 节点，成功创建的 Broker 成为
Controller。这是通过 ZooKeeper 的 <code v-pre>create</code> 接口实现的，如果节点已经存在，则创建失败。</p>
...
<h4 id="partition-分配过程" tabindex="-1"><a class="header-anchor" href="#partition-分配过程"><span>Partition 分配过程</span></a></h4>
<p>一旦 Controller 选举完成，新的 Controller 会承担管理集群范围内的任务，包括 Partition 分配。
这是通过 Kafka 内部逻辑实现的，而不是在 ZooKeeper 中直接触发。</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="分区状态机" tabindex="-1"><a class="header-anchor" href="#分区状态机"><span>分区状态机</span></a></h4>
<p>Kafka 使用分区状态机和副本状态机来管理 Partition 和 Replica 的状态。在启动过程中，新的
Controller 会调用 <code v-pre>partitionStateMachine</code> 和 <code v-pre>replicaStateMachine</code> 来处理分区和副本
的状态变化。</p>
<h3 id="_4-zookeeper-和-kafka-的协作" tabindex="-1"><a class="header-anchor" href="#_4-zookeeper-和-kafka-的协作"><span>4. ZooKeeper 和 Kafka 的协作</span></a></h3>
<p>在 Kafka 的设计中，ZooKeeper 主要负责协调和存储元数据，而实际的业务逻辑和处理是在 Kafka 内
部实现的。具体来说：</p>
<ul>
<li><strong>ZooKeeper</strong>：存储和管理 Broker 列表、Topic 列表、Partition 信息、Controller 节点等元数据。</li>
<li><strong>Kafka Controller</strong>：负责具体的管理任务，包括 Partition 分配、Leader 选举等。</li>
</ul>
<h3 id="原理示例" tabindex="-1"><a class="header-anchor" href="#原理示例"><span>原理示例</span></a></h3>
<h4 id="启动-kafka-broker" tabindex="-1"><a class="header-anchor" href="#启动-kafka-broker"><span>启动 Kafka Broker</span></a></h4>
<p>每个 Kafka Broker 启动时，会尝试创建 <code v-pre>/controller</code> 节点。如果成功创建，该 Broker 成为 Controller。</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="controller-分配-partition" tabindex="-1"><a class="header-anchor" href="#controller-分配-partition"><span>Controller 分配 Partition</span></a></h4>
<p>新选举的 Controller 负责分配 Partition 的 Leader 和 Follower，并更新 ZooKeeper 中的元数据。</p>
...
<p>Kafka 集群在启动过程中，通过 ZooKeeper 协调各个 Broker 的工作。ZooKeeper 负责选举 Controller，
成功创建 <code v-pre>/controller</code> 节点的 Broker 成为 Controller。Controller 负责管理集群范围内的任务，包
括 Partition 分配和 Leader 选举。这些操作主要通过 Kafka 内部的逻辑实现，而不是直接在 ZooKeeper
中触发。理解这些原理和实现细节，有助于架构师更好地设计和维护 Kafka 系统。</p>
</div></template>
